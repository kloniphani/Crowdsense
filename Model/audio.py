...
class augmentation():
    ...
    def plot_waveform(audio, title="Waveform", xlim=None, ylim=None, file_name=None, show=False):
        ...
        if file_name is not None:
            ...
            plt.savefig(f'{Path.cwd()}\\Results\{file_name[-1].split(".")[0]} - {title} - Waveform.png', format="png")
        if show == True:
            plt.show(block=True)
        plt.close()
    ...
    def plot_specgram(audio, title="Spectrogram", xlim=None, file_name=None, show=False):
        ...
        if file_name is not None:
            file_name = file_name.split('\\')
            plt.savefig(f'{Path.cwd()}\\results\{file_name[-1].split(".")[0]} - {title} - Spectrogram.png', format="png")
        if show == True:
            plt.show(block=True)
        plt.close()
...
if __name__ == "__main__":
    ...
    reaud = augmentation.resample(Audio, sr)
    ...
    rechan = augmentation.rechannel(reaud, channel)
    ...
    dur_aud = augmentation.pad_trunc(rechan, duration)
    ...
    shift_aud = augmentation.time_shift(dur_aud, shift_pct)
    ...
    effects_aud = augmentation.effect_filtering(
    ...
    background_aud = augmentation.background_noise(effects_aud, snr_db=10)
    ...
    codec_aud = augmentation.codecs(background_aud)
    ...
    sgram = augmentation.spectro_gram(
    ...
    aug_sgram = augmentation.spectro_augment(sgram, max_mask_pct=0.1, n_freq_masks=2, n_time_masks=2)
